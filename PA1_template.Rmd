...
```
The **`r stepsinterval[which.max(stepsinterval$steps),]$interval` interval** contains the maximum number of steps on average across all the days.
...
Total number of rows with NAs is `r sum(!complete.cases(data))`
